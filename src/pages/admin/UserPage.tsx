        ...
        try {
            const res = await axios.get(API_URL, {
                ...
                params: {
                    ...
                    planName: filterPlan === "ALL" ? undefined : filterPlan,
                    expiring: filterExpiring ? true : undefined,
                },
            });
            ...
            <Card className="shadow-none border-none bg-transparent rounded-none">
                ...
                <CardContent>
                    ...
                    <div className="flex flex-wrap gap-4 mb-4 items-center">
                        ...
                        {/* Lọc gói dịch vụ */}
                        <Select onValueChange={(val) => setFilterPlan(val)}>
                            ...
                            <SelectContent>
                                ...
                                <SelectItem value="THREE_YEAR">3 năm</SelectItem>
                            </SelectContent>
                        </Select>

                        {/* Checkbox lọc gần hết hạn */}
                        ...
                        <TableBody>
                            {users.map((u) => (
                                <TableRow key={u.id}>
                                    ...
                                    <TableCell>{u.planStartDate ? new Date(u.planStartDate).toLocaleDateString() : "-"}</TableCell>
                                    <TableCell>
                                        {u.planEndDate ? (
                                            <span
                                                className={
                                                    new Date(u.planEndDate).getTime() - Date.now() < 7 * 24 * 60 * 60 * 1000
                                                        ? "text-red-600 font-bold"
                                                        : ""
                                                }
                                            >
                                                {new Date(u.planEndDate).toLocaleDateString()}
                                            </span>
                                        ) : (
                                            "-"
                                        )}
                                    </TableCell>
                                    <TableCell>{u.planPrice ? u.planPrice.toLocaleString("vi-VN") + " ₫" : "-"}</TableCell>
                                    <TableCell>
                                        {u.paid ? (
